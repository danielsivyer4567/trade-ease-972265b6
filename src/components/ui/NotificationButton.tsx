...
export function NotificationButton() {
  ...
  return <>
      <Button onClick={() => setOpen(true)} className={cn("rounded-full w-14 h-14 bg-blue-500 hover:bg-blue-600 fixed bottom-6 right-6 flex items-center justify-center shadow-lg z-50", isPinned && "opacity-0 pointer-events-none")} aria-label="Notifications">
        <Bell className="h-6 w-6 text-white" />
        {unreadCount > 0 && <span className="absolute top-0 right-0 h-5 w-5 bg-red-500 rounded-full text-xs text-white flex items-center justify-center">
            {unreadCount}
          </span>}
      </Button>
            ...
            <SheetHeader className="px-6 pt-6 pb-2 border-b">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" onClick={handleTogglePin} className="rounded-full h-8 w-8 flex items-center justify-center" title={isPinned ? "Unpin notifications" : "Pin notifications"}>
            ...
            <Tabs defaultValue="all" className="flex-1 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto px-4">
                <TabsContent value="all" className="mt-0 space-y-4">
                  {notifications.map(notification => <div key={notification.id} className="flex items-start gap-3 py-3 border-b border-gray-100 cursor-pointer hover:bg-gray-50 px-2" onClick={() => handleNotificationClick(notification.id)}>
                      ...
                      {!notification.isCompleted && <div className="w-2 h-2 rounded-full bg-blue-600 flex-shrink-0 mt-2"></div>}
                    </div>)}
