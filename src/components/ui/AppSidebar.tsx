...
export function AppSidebar() {
  ...
  const renderNavLink = (icon: React.ReactNode, label: string, path: string, openInNewTab: boolean = false) => {
    ...
    const content = <Link to={path} className={cn(
        ...
        isMobileDevice && "p-3", // Larger touch target on mobile
      )} {...linkProps}>
        {icon}
  ...
  return <>
      ...
      <div data-state={state} className="peer z-30">
        <div className="flex flex-col h-full bg-[#181835]">
          ...
          <nav className="flex-1 p-2 space-y-1 overflow-y-auto bg-slate-200">
            ...
            <div className={cn("pt-8", isCollapsed && "hidden lg:hidden")}>
              ...
              {teams.map(team => <Link key={team.name} to={team.path} className={cn("flex items-center gap-2 p-2 rounded-lg text-sm", `hover:bg-${team.color}-50 text-${team.color}-600 hover:text-${team.color}-700`, isMobileDevice && "p-3")}>
                  <Hammer className="w-4 h-4 flex-shrink-0" />
                  <span className={cn("transition-opacity duration-200", isCollapsed && "hidden lg:hidden")}>
                    {team.name}
                  </span>
                </Link>)}
              <Link to="/team-new" className={cn("flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 text-gray-600 hover:text-gray-700 text-sm mt-1", isCollapsed && "hidden lg:hidden", isMobileDevice && "p-3")}>
