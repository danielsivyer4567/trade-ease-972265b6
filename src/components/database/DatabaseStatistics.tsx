...
export function DatabaseStatistics() {
    ...
    async function loadStatistics() {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('statistics_history').select('entity_type, count').eq('count_date', new Date().toISOString().split('T')[0]);
  ...
  return <div className="space-y-6">
      ...
      {loading ? <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5].map(i => <Card key={i} className="bg-slate-100 animate-pulse">
        ...
        </div> : <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-100">
          {statistics.length === 0 ? <p className="text-gray-500 col-span-full">No statistics available</p> : statistics.map(stat => <Card key={stat.entity_type} className="bg-slate-100 hover:shadow-md transition-shadow">
