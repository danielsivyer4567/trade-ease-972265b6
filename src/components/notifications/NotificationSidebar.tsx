...
export const NotificationSidebar = ({ isOpen, onClose }: NotificationSidebarProps) => {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed right-0 top-0 h-screen bg-white z-50 shadow-lg w-[350px] transform transition-transform duration-300 ease-in-out",
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex rounded-lg bg-gray-100 p-1 mx-4 my-3">
            <button
              className={cn(
                "flex-1 text-center py-2 rounded-md transition-all",
            ...
            <button
              className={cn(
                "flex-1 text-center py-2 rounded-md transition-all",
                activeTab === 'team' ? "bg-white shadow" : "hover:bg-gray-200"
              )}
          ...
          <div className="flex-1 overflow-y-auto divide-y">
            {activeTab === 'all' && mockNotifications.map(notification => (
              <NotificationItem
                key={notification.id}
                {...notification}
              />
