...
export function TeamCalendar({
  ...
  return <section>
      <div className="p-6 rounded-lg shadow-md bg-slate-300 my-[32px] px-0 py-[31px] mx-px">
        <Calendar mode="single" selected={date} onSelect={setDate} modifiers={modifiers} modifiersStyles={modifiersStyles} components={{
        DayContent: ({
          date
        }) => {
          const dateStr = format(date, 'yyyy-MM-dd');
          ...
          return <div className="relative w-full h-full flex items-center justify-center" onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, date)}>
                  ...
                  {weatherData && <>
                      ...
                      {weatherData.rainfall > 0 && <div className="absolute bottom-1 text-[10px] text-blue-500 font-medium">
                          {weatherData.rainfall}mm
                        </div>}
                    </>}
                  {jobsForDate && jobsForDate.length > 0 && <div className="absolute bottom-1 right-1 bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">
                      {jobsForDate.length}
                    </div>}
                </div>;
        }
