...
const MAPBOX_ACCESS_TOKEN = import.meta.env.VITE_MAPBOX_ACCESS_TOKEN || 'pk.eyJ1IjoiZGFuaWVsc2l2eWVyIiwiYSI6ImNtYzFjNjVmbzBiOGwybW9oZ203cXdxZGMifQ.heFI2tJzQDKcGNLwgGH2Kw'
...
export default function ParcelMap() {
  ...
  useEffect(() => {
    if (!MAPBOX_ACCESS_TOKEN) {
      setMapError('Mapbox access token is not configured. Please check your environment variables.')
      return
    }
    ...
    try {
      ...
      map.on('load', () => {
        ...
        map.on('click', 'parcels-fill', (e: any) => {
          ...
          const isSelected = selectedFeatures.has(id as string)
          ...
          if (isSelected) {
            selectedFeatures.delete(id as string)
            edgePopups.get(id as string)?.forEach(p => p.remove())
            edgePopups.delete(id as string)
            ...
            edgePopups.set(id as string, popups)
