...
export function TaskList({
  ...
  const handleInProgress = (taskId: string) => {
    ...
    onProgress(taskId, progressNote, progressFiles);
    setProgressNote("");
    setProgressFiles([]);
  ...
  return (
    <div className="grid gap-4">
      {tasks.filter(task => task.assignedTeam === teamName).map((task, index) => (
        <Collapsible 
          key={task.id} 
            ...
            <CardHeader className="relative px-[43px] my-px bg-slate-300">
              <CollapsibleTrigger className="w-full text-left flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {openTaskId === task.id ? 
                    <ChevronUp className="h-4 w-4 text-gray-500 bg-slate-400" /> : 
            ...
            <CollapsibleContent>
              <CardContent className="space-y-6 bg-slate-300">
                ...
                {task.attachedFiles && task.attachedFiles.length > 0 && (
                  <ImagesGrid images={task.attachedFiles} title="Attached Files" />
                )}
