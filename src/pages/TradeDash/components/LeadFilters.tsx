...
export const LeadFilters: React.FC<FiltersProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {showAdvancedFilters && (
        <div className="space-y-4 bg-slate-100 p-4 rounded-md">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input 
                    id="postcode-filter"
                    placeholder="Enter postcodes and press Enter"
                    onKeyPress={handlePostcodeKeyPress}
                  />
                  {selectedPostcodes.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedPostcodes.map(postcode => (
                        <div key={postcode} className="flex items-center bg-slate-200 px-2 py-1 rounded-md text-xs">
                          {postcode}
                          <X 
              ...
              <div className="flex flex-col md:flex-row gap-4 pt-2 border-t border-gray-200">
                <div className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => setIncludeResold(!!checked)}
                  />
                  <label
                    htmlFor="include-resold"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox 
                    id="contractor-leads" 
                    checked={includeContractorLeads}
                    onCheckedChange={(checked) => setIncludeContractorLeads(!!checked)}
                  />
                  <label
                    htmlFor="contractor-leads"
                    className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                  >
