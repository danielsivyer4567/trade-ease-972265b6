...
export const JobHeader = ({
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(`${WEBSOCKET_BASE_URL}/jobs/${job.id}?userId=${userId}&token=${authToken}`);
    socket.onopen = () => {
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-2 py-0 mx-0 my-0 px-[232px]">
            <Button variant="outline" size="icon" onClick={toggleListening} className="rounded-full">
              {isListening ? <Volume2 /> : <VolumeX />}
            </Button>
            <Button className={`bg-red-500 hover:bg-red-600 text-white font-bold uppercase tracking-wider flex items-center gap-2 min-w-[150px] h-12 ${isOnSmoko ? 'animate-pulse' : ''}`} onClick={handleSmoko}>
              <Coffee className={isOnSmoko ? 'animate-bounce' : ''} />
              SMOKO
            </Button>
            <Button className="bg-red-700 hover:bg-red-800 text-white font-bold uppercase tracking-wider flex items-center gap-2 min-w-[150px] h-12" onClick={handlePackUp}>
              <PackageX />
              PACK HER UP
            </Button>
            <Button className="bg-green-500 hover:bg-green-600 text-white font-bold uppercase tracking-wider flex items-center gap-2 min-w-[200px] h-12" onMouseDown={startRecording} onMouseUp={stopRecording} onMouseLeave={stopRecording} onTouchStart={startRecording} onTouchEnd={stopRecording}>
